...
def analyze_complexity(input_sizes: List[int]):
    ...
    # Plot data aktual
    plt.plot(input_sizes, quicksort_times, 'bo-', label='Quicksort')
    plt.plot(input_sizes, mergesort_times, 'ro-', label='Mergesort')
    
    # Plot fungsi n log n (diskala untuk visualisasi)
    x = np.array(input_sizes)
    nlogn = x * np.log2(x)
    scale_factor = np.mean(quicksort_times) / np.mean(nlogn)
    plt.plot(x, nlogn * scale_factor, 'g--', label='O(n log n)')
    
    plt.xlabel('Ukuran Input (n)')
    plt.ylabel('Waktu Eksekusi (detik)')
    plt.title('Analisis Kompleksitas Waktu Quicksort vs Mergesort')
    plt.legend()
    plt.grid(True)
    plt.xscale('log')
    plt.yscale('log')
